...
struct SigfmImgInfo
{
  ...
  int width;
  int height;
  uint8_t *pix;  /* normalised 8-bit image used for descriptor extraction */
};
...
static gsize brief_init_done = 0;

static void
init_brief_pattern (void)
{
  if (g_once_init_enter (&brief_init_done))
    {
      ...
      g_once_init_leave (&brief_init_done, 1);
    }
}
                    ...
                    int border)
{
  ...
  int *scores = g_malloc0((gsize)(w * h) * sizeof(int));
...
static int
estimate_transform_internal(const SigfmImgInfo *frame, const SigfmImgInfo *enrolled,
                            SigfmTransform *out)
{
  ...
  const float eps_sq = RANSAC_INLIER_THRESH * RANSAC_INLIER_THRESH;
  int best_inliers = 0;
  float best_a = 1, best_b = 0, best_c = 0, best_d = 1, best_tx = 0, best_ty = 0;
  uint32_t rng = 2654435761u;
  ...
  for (int iter = 0; iter < RANSAC_ITERATIONS; iter++)
    {
      int i1 = (int)(xorshift32(&rng) % (uint32_t)n);
      int i2 = (int)(xorshift32(&rng) % (uint32_t)(n - 1));
      if (i2 >= i1)
        i2++;

      float p1x = frame->kp[matches[i1].qi].x, p1y = frame->kp[matches[i1].qi].y;
      float p2x = frame->kp[matches[i2].qi].x, p2y = frame->kp[matches[i2].qi].y;
      float q1x = enrolled->kp[matches[i1].ti].x, q1y = enrolled->kp[matches[i1].ti].y;
      float q2x = enrolled->kp[matches[i2].ti].x, q2y = enrolled->kp[matches[i2].ti].y;
      ...
      int inliers = count_inliers(frame, enrolled, matches, n,
                                  cos_t, -sin_t, sin_t, cos_t, tx, ty, eps_sq);
      if (inliers > best_inliers)
        {
          best_inliers = inliers;
          best_a = cos_t; best_b = -sin_t; best_c = sin_t; best_d = cos_t;
  ...
  if (best_inliers >= 3)
    {
      float a, b, cc, dd, tx, ty;
      int refined = ls_refine(frame, enrolled, matches, n,
                              best_a, best_b, best_c, best_d, best_tx, best_ty,
                              eps_sq, &a, &b, &cc, &dd, &tx, &ty);
      if (refined >= best_inliers)
        {
          best_inliers = refined;
          best_a = a; best_b = b; best_c = cc; best_d = dd;
          best_tx = tx; best_ty = ty;
        }
    }
  g_free(matches);

  if (out && best_inliers > 0)
    {
      out->a = best_a; out->b = best_b; out->c = best_c; out->d = best_d;
      out->tx = best_tx; out->ty = best_ty; out->inliers = best_inliers;
    }
  return best_inliers;
}

int
sigfm_pixel_overlap_metrics(SigfmImgInfo *probe, SigfmImgInfo *enrolled,
                            int *out_inliers, int *out_overlap,
                            int *out_zncc_milli, int *out_agree_permille)
{
  if (out_inliers) *out_inliers = 0;
  if (out_overlap) *out_overlap = 0;
  if (out_zncc_milli) *out_zncc_milli = 0;
  if (out_agree_permille) *out_agree_permille = 0;

  if (!probe || !enrolled || !probe->pix || !enrolled->pix ||
      probe->width <= 0 || probe->height <= 0 ||
      probe->width != enrolled->width || probe->height != enrolled->height)
    return 0;

  SigfmTransform tr = {0};
  int inliers = estimate_transform_internal(probe, enrolled, &tr);
  if (out_inliers)
    *out_inliers = inliers;
  if (inliers < 2)
    return 0;
  ...
  for (int y = 1; y < h - 1; y++)
    for (int x = 1; x < w - 1; x++)
      {
        float qxf = tr.a * x + tr.b * y + tr.tx;
        float qyf = tr.c * x + tr.d * y + tr.ty;
        int qx = (int)lrintf(qxf);
        int qy = (int)lrintf(qyf);
        if (qx < 1 || qx >= w - 1 || qy < 1 || qy >= h - 1)
          continue;

        double pv = probe->pix[y * w + x];
        double qv = enrolled->pix[qy * w + qx];
        sx += pv; sy += qv;
        sxx += pv * pv; syy += qv * qv; sxy += pv * qv;
        n++;
      }
  ...
  double zncc = (vx > 1e-9 && vy > 1e-9) ? cov / sqrt(vx * vy) : 0.0;
  zncc = CLAMP(zncc, -1.0, 1.0);
    ...
    for (int x = 1; x < w - 1; x++)
      {
        ...
        if (qx < 1 || qx >= w - 1 || qy < 1 || qy >= h - 1)
          continue;
        gboolean a = probe->pix[y * w + x] >= mx;
        gboolean b = enrolled->pix[qy * w + qx] >= my;
        if (a == b)
          agree++;
      }

  if (out_zncc_milli)
    *out_zncc_milli = (int)lrint(zncc * 1000.0);
  if (out_agree_permille)
    *out_agree_permille = (int)((1000LL * agree + n / 2) / n);
  return 1;
}
...
sigfm_extract(const SigfmPix *pix, int width, int height)
{
  ...
  uint8_t *blurred = g_malloc((gsize)(width * height));
  ...
  if (hw >= 16 && hh >= 16 && n < MAX_KP)
    {
      uint8_t *half = g_malloc((gsize)(hw * hh));
  ...
  SigfmImgInfo *info = g_new (SigfmImgInfo, 1);

  info->n_kp = n;
  info->kp = g_malloc((gsize)n * sizeof(OrbKeypoint));
  info->desc = g_malloc((gsize)n * DESC_BYTES);
  info->width = width;
  info->height = height;
  info->pix = g_memdup2(pix, (gsize)width * height);
...
sigfm_free_info(SigfmImgInfo *info)
{
  ...
  if (info->pix)
    {
      memset(info->pix, 0, (gsize)info->width * info->height);
      g_free(info->pix);
    }
  g_free(info);
}
...
sigfm_copy_info(SigfmImgInfo *info)
{
  ...
  SigfmImgInfo *copy = g_new (SigfmImgInfo, 1);

  copy->n_kp = info->n_kp;
  copy->kp = g_malloc((gsize)info->n_kp * sizeof(OrbKeypoint));
  copy->desc = g_malloc((gsize)info->n_kp * DESC_BYTES);
  copy->width = info->width;
  copy->height = info->height;
  copy->pix = info->pix && info->width > 0 && info->height > 0
                ? g_memdup2(info->pix, (gsize)info->width * info->height)
                : NULL;
 ...
 *   [4]  version = 3  (v2 had features only)
 *   [4]  n_kp
 *   [4]  width
 *   [4]  height
 *   [n_kp * 12]  keypoints: float x, float y, float response
 *   [n_kp * 32]  descriptors
 *   [width * height] normalised 8-bit enrollment image
 * ---------------------------------------------------------------------- */

#define SIGFM_MAGIC 0x4D474653u
#define SIGFM_VERSION 3u
...
sigfm_serialize_binary(SigfmImgInfo *info, int *outlen)
{
  ...
  if (!info->pix || info->width <= 0 || info->height <= 0)
    return NULL;

  int kp_bytes = info->n_kp * (int)(3 * sizeof(float));
  int desc_bytes = info->n_kp * DESC_BYTES;
  int pix_bytes = info->width * info->height;
  int total = 20 + kp_bytes + desc_bytes + pix_bytes; /* 5 × u32 header */

  uint8_t *buf = g_malloc((gsize)total);

  write_u32_le(buf + 0, SIGFM_MAGIC);
  write_u32_le(buf + 4, SIGFM_VERSION);
  write_u32_le(buf + 8, (uint32_t)info->n_kp);
  write_u32_le(buf + 12, (uint32_t)info->width);
  write_u32_le(buf + 16, (uint32_t)info->height);
  ...
  p += desc_bytes;
  memcpy(p, info->pix, (size_t)pix_bytes);
...
sigfm_deserialize_binary(const unsigned char *bytes, int len)
{
  ...
  int width = (int)read_u32_le(bytes + 12);
  int height = (int)read_u32_le(bytes + 16);
  if (n_kp <= 0 || n_kp > MAX_KP || width <= 0 || height <= 0 ||
  ...
  if (len != 20 + kp_bytes + desc_bytes + pix_bytes)
    return NULL;

  SigfmImgInfo *info = g_new0 (SigfmImgInfo, 1);

  info->n_kp = n_kp;
  info->kp = g_malloc((gsize)n_kp * sizeof(OrbKeypoint));
  info->desc = g_malloc((gsize)desc_bytes);
  info->width = width;
  info->height = height;
  info->pix = g_malloc((gsize)pix_bytes);

  const uint8_t *p = bytes + 20;
  ...
  p += desc_bytes;
  memcpy(info->pix, p, (size_t)pix_bytes);
